...
export default function TrendingPage() {
  ...
  useEffect(() => {
    async function fetchScripts() {
      ...
      try {
        fetch(`/api/leaderboard?limit=100&_t=${Date.now()}`, { cache: 'no-store' })
          .then(r => r.json())
          .then(data => {
            const arr = Array.isArray(data.users) ? data.users : [];
            if (arr.length > 0) {
              sessionStorage.setItem('leaderboard_top100', JSON.stringify({ leaders: arr, timestamp: Date.now() }));
            }
          })
          .catch(() => {});
      } catch {}
      try {
        const res = await fetch(`/api/scripts/fast?limit=32&includePopular=1`);
        const data = await res.json();
        const list = Array.isArray(data.popular) ? data.popular : (Array.isArray(data.newest) ? data.newest : []);
        setScripts(list.length > 0 ? list : DEMO_SCRIPTS);
      } catch (e) {
    ...
    <div className="min-h-screen bg-gray-900 text-white">
        ...
        <div className="max-w-7xl mx-auto w-full mb-8">
          ...
          <div className="grid grid-cols-2 w-full rounded-xl overflow-hidden shadow-lg mb-6">
            <button className="w-full px-6 py-3 text-sm sm:text-base font-semibold bg-white text-gray-900 focus:outline-none cursor-default">Trending Scripts</button>
            <Link href="/leaderboard" className="w-full px-6 py-3 text-sm sm:text-base font-semibold bg-white/10 backdrop-blur text-white border-l border-white/20 hover:bg-white/20 transition-colors text-center">User Leaderboard</Link>
          </div>

          <h1 className="text-center text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight">Trending Scripts</h1>
        </div>

        <div className="w-full mb-3">
          {loading ? null : visibleScripts.length === 0 ? null : (
